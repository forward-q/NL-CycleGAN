...
class CinCGAN():
    def __init__(self, gpu, ngpus_per_node, args):
        ...
        if os.path.isdir('/mnt/workspace/CinCGAN-pytorch/CinCGAN_pytorch/target_fid_statistics'):
            self.fid_mean = np.load(
                os.path.join('/mnt/workspace/CinCGAN-pytorch/CinCGAN_pytorch/target_fid_statistics', 'm2.npy'))
            self.fid_cov = np.load(
                os.path.join('/mnt/workspace/CinCGAN-pytorch/CinCGAN_pytorch/target_fid_statistics', 's2.npy'))
    ...
    def build_model(self):
        ...
        if self.ngpus_per_node > 1:
            self.G1_forward = torch.nn.parallel.DistributedDataParallel(self.G1_forward, device_ids=[self.gpu],
                                                                        broadcast_buffers=False)
            self.G1_backward = torch.nn.parallel.DistributedDataParallel(self.G1_backward, device_ids=[self.gpu],
                                                                         broadcast_buffers=False)
            self.D1_forward = torch.nn.parallel.DistributedDataParallel(self.D1_forward, device_ids=[self.gpu],
                                                                        broadcast_buffers=False)
            self.D1_backward = torch.nn.parallel.DistributedDataParallel(self.D1_backward, device_ids=[self.gpu],
                                                                         broadcast_buffers=False)
        ...
        """ Inner Optimizers """
        self.G_optim = torch.optim.Adam(itertools.chain(self.G1_forward.parameters(), self.G1_backward.parameters()),
                                        lr=self.lr, betas=(0.5, 0.999), eps=self.eps)
        self.D_optim = torch.optim.Adam(itertools.chain(self.D1_forward.parameters(), self.D1_backward.parameters()),
                                        lr=self.lr, betas=(0.5, 0.999), eps=self.eps)

        """ Outer Optimizers """
        self.G_outer_optim = torch.optim.Adam(
            itertools.chain(self.G3.parameters(), self.G1_forward.parameters(), self.EDSR.parameters()),
            lr=self.lr_outer, betas=(0.5, 0.999), eps=self.eps)
        ...
        self.train_t_dataset = DatasetFolder(self.train_t_path, 1, transform=None, return_two_img=True,
                                             big_imsize=self.imsize_y, scale_factor=self.scale_factor)
        ...
        if self.ngpus_per_node > 1:
            train_s_sampler = torch.utils.data.distributed.DistributedSampler(self.train_s_dataset,
                                                                              num_replicas=self.ngpus_per_node,
                                                                              rank=self.gpu)
            train_t_sampler = torch.utils.data.distributed.DistributedSampler(self.train_t_dataset,
                                                                              num_replicas=self.ngpus_per_node,
                                                                              rank=self.gpu)
            self.train_s_loader = DataLoader(self.train_s_dataset, batch_size=self.batch_size, shuffle=False,
                                             num_workers=self.num_workers, sampler=train_s_sampler)
            self.train_t_loader = DataLoader(self.train_t_dataset, batch_size=self.batch_size, shuffle=False,
                                             num_workers=self.num_workers, sampler=train_t_sampler)
        else:
            self.train_s_loader = DataLoader(self.train_s_dataset, batch_size=self.batch_size, shuffle=False,
                                             num_workers=self.num_workers)
            self.train_t_big_loader = DataLoader(self.train_t_big_dataset, batch_size=self.batch_size, shuffle=False,
                                                 num_workers=self.num_workers)
            self.train_t_loader = DataLoader(self.train_t_dataset, batch_size=self.batch_size, shuffle=False,
                                             num_workers=self.num_workers)
    ...
    def train(self, inner=True):
        ...
        for model in [self.G1_forward, self.G1_backward, self.D1_forward, self.D1_backward, self.EDSR, self.D2,
                      self.G3]:
            model.train()
        if self.skip_inner:
        ...
        # Validation data
        x_test, _ = next(iter(self.test_s_loader))

        x_test = x_test.to(self.device)

        y_test_big, _ = next(iter(self.test_t_loader))

        # if self.use_fid == False:
        #     assert x_test.size()[0] == y_test_big.size()[0] == 1
        y_test = transforms.Resize(x_test.size()[-2:], interpolation=Image.BICUBIC)(
            tensor2pil(y_test_big[0]))  # To resize, tensor should be converted to pillow image.
        ...
        for step in range(start_iter, self.iteration + 1):
            ...
            if step % self.decay_step == 0:
                ...
                self.G_optim.param_groups[0][
                    'lr'] = self.lr / 2 ** n  # param_groups = [{'amsgrad': False, 'betas': (...), 'eps': 1e-08, 'lr': 0.0002, 'params': [...], 'weight_decay': 0}]
            ...
            try:
                x, label_x = next(train_s_iter)
            except:
                train_s_iter = iter(self.train_s_loader)
                x, label_x = next(train_s_iter)

            try:
                y_big, y, label_y = next(train_t_iter)
            ...
            if self.skip_inner == False:
                ...
                print(
                    f"dis1_forward_output_fake.shape : {dis1_forward_output_fake.size()} dis1_backward_output_fake.shape : {dis1_backward_output_fake.size()} ")
                ...
                # Discriminator loss
                dis1_forward_ad_loss = self.MSE_loss(dis1_forward_predicted_fake,
                                                     torch.zeros_like(dis1_forward_predicted_fake).to(
                                                         self.device)) + self.MSE_loss(dis1_forward_predicted_real,
                                                                                       torch.ones_like(
                                                                                           dis1_forward_predicted_real).to(
                                                                                           self.device))
                dis1_backward_ad_loss = self.MSE_loss(dis1_backward_predicted_fake,
                                                      torch.zeros_like(dis1_backward_predicted_fake).to(
                                                          self.device)) + self.MSE_loss(dis1_backward_predicted_real,
                                                                                        torch.ones_like(
                                                                                            dis1_backward_predicted_real).to(
                                                                                            self.device))
                ...
                # Adversarial loss
                g1_forward_ad_loss = self.MSE_loss(dis1_forward_predicted_fake,
                                                   torch.ones_like(dis1_forward_predicted_fake).to(self.device))
                g1_backward_ad_loss = self.MSE_loss(dis1_backward_predicted_fake,
                                                    torch.ones_like(dis1_backward_predicted_fake).to(self.device))
                ...
                if inner == True:
                    ...
                    print(
                        f"step : {step}  d_loss : {dis1_loss}  g_loss : {g1_loss}  lr : {self.D_optim.param_groups[0]['lr']}")
            # get inner psnr
            ...
            if inner == True:
                if step % 50 == 0:
                    tensor_imsave(img_t, f'res_imgs', f'img{step}.png')
            ...
            # Check D2
            print(
                f"D2 prediction Avg -- fake : {torch.mean(d2_predicted_fake):0.4f}  real : {torch.mean(d2_predicted_real):0.4f}")

            d2_ad_loss = self.MSE_loss(d2_predicted_fake,
                                       torch.zeros_like(d2_predicted_fake).to(self.device)) + self.MSE_loss(
                d2_predicted_real, torch.ones_like(d2_predicted_real))
            ...
            with torch.no_grad():
                if step % 50 == 0:
                    ...
                    try:
                        ...
                        out = self.EDSR(out)
                        tensor_imsave(out[0], 'imgs_outer', f'{step}.png', denormalization=False)
                    except:
                        None
            if step % self.save_freq == 0:
                self.save()
    ...
    def test(self, inner=True, idx=0):
        ...
        for img, fname in zip(fake_outer, x_name):
            tensor_imsave(fake_outer[0], 'test', fname, denormalization=False)
        if self.use_psnr:
        ...
        with torch.no_grad():
            while True:
                ...
                try:
                    ...
                    break
                y = transforms.Resize(x.size()[-2:], interpolation=Image.BICUBIC)(
                    tensor2pil(y_big[0]))  # To resize, tensor should be converted to pillow image.
    ...
    def inference(self):  # SR inference for whole data
        ...
        for x, x_name in test_iter:
            ...
            for img in out:
                ...
                try:
                    tensor_imsave(img, f"trainval", f"{x_name[0]}", denormalization=False)
                except:
                    tensor_imsave(img, f"trainval", f"{x_name}", denormalization=False)
